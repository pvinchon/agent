...
func loopFlags(fs *flag.FlagSet) (mustReviewers func() []reviewer.Reviewer, mustAssistant func() assistant.Assistant, resolveLog func() *slog.Logger, maxAttempts *int, reviewTemplate func() string, fixTemplate func() string) {
maxAttempts = fs.Int("max-attempts", 5, "maximum number of fix attempts")
return reviewer.FlagSet(fs), assistant.FlagSet(fs), xlog.FlagSet(fs), maxAttempts, reviewer.TemplateFlagSet(fs), fixer.TemplateFlagSet(fs)
}
