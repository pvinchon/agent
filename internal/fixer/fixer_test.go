...
diff := "my diff"
custom := "Fix these: {{issues}}\nCode: {{diff}}"

prompt := buildPrompt(testIssues, diff, custom)
...
// Custom template replaces the built-in preamble.
if strings.Contains(prompt, "You are a senior engineer") {
...
func TestFix_customTemplate(t *testing.T) {
var capturedPrompt string
a := &fakeAssistant{fn: func(prompt string) *exec.Cmd {
capturedPrompt = prompt
return echoCmd("")
...
custom := "Custom fixer: {{issues}} Diff: {{diff}}"
if err := Fix(testIssues, "diff", custom, a); err != nil {
t.Fatalf("unexpected error: %v", err)
}
if !strings.Contains(capturedPrompt, "SQL injection") {
t.Error("prompt does not contain issue title")
}
if strings.Contains(capturedPrompt, "You are a senior engineer") {
t.Error("prompt should not contain built-in preamble when custom template is used")
}
}
